...
func (s Int64s) GetContext() *aargh.Context {
	return s.Ctx_
}
...
func (s Int64s) IsSorted() aargh.SeriesSortOrder {
	return s.Sorted_
}
...
func (s Int64s) Append(v any) Series {
	...
	case NAs:
		...
		s.Data_ = append(s.Data_, make([]int64, v.Len())...)
	...
	case []aargh.NullableInt64:
		ssize := len(s.Data_)
		s.Data_ = append(s.Data_, make([]int64, len(v))...)
		s = s.MakeNullable().(Int64s)
		if len(s.Data_) > len(s.NullMask_)<<3 {
...
func (s Int64s) filterBoolSlice(mask []bool) Series {
	if len(mask) != len(s.Data_) {
		return Errors{fmt.Sprintf("Int64s.Filter: mask length (%d) does not match series length (%d)", len(mask), len(s.Data_))}
	}
...
func (s Int64s) Map(f aargh.MapFunc) Series {
	if len(s.Data_) == 0 {
		return s
	}
...
// Apply the given function to each element of the series.
func (s Int64s) MapNull(f aargh.MapFuncNull) Series {
	if len(s.Data_) == 0 {
		return s
	}
