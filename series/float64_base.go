...
func (s Float64s) GetContext() *aargh.Context {
	return s.Ctx_
}
...
func (s Float64s) IsSorted() aargh.SeriesSortOrder {
	return s.Sorted_
}
...
func (s Float64s) Map(f aargh.MapFunc) Series {
	if len(s.Data_) == 0 {
		return s
	}
...
// Apply the given function to each element of the series.
func (s Float64s) MapNull(f aargh.MapFuncNull) Series {
	if len(s.Data_) == 0 {
		return s
	}
