...
func (s Ints) GetContext() *aargh.Context {
	return s.Ctx_
}
...
func (s Ints) IsSorted() aargh.SeriesSortOrder {
	return s.Sorted_
}
...
func (s Ints) Append(v any) Series {
	...
	case NAs:
		...
		s.Data_ = append(s.Data_, make([]int, v.Len())...)
	...
	case []aargh.NullableInt:
		ssize := len(s.Data_)
		s.Data_ = append(s.Data_, make([]int, len(v))...)
		s = s.MakeNullable().(Ints)
		if len(s.Data_) > len(s.NullMask_)<<3 {
...
func (s Ints) filterBoolSlice(mask []bool) Series {
	if len(mask) != len(s.Data_) {
		return Errors{fmt.Sprintf("Ints.Filter: mask length (%d) does not match series length (%d)", len(mask), len(s.Data_))}
	}
...
func (s Ints) Map(f aargh.MapFunc) Series {
	if len(s.Data_) == 0 {
		return s
	}
...
// Apply the given function to each element of the series.
func (s Ints) MapNull(f aargh.MapFuncNull) Series {
	if len(s.Data_) == 0 {
		return s
	}
