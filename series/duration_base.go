...
func (s Durations) GetContext() *aargh.Context {
	return s.Ctx_
}
...
func (s Durations) IsSorted() aargh.SeriesSortOrder {
	return s.Sorted_
}
...
func (s Durations) Map(f aargh.MapFunc) Series {
	if len(s.Data_) == 0 {
		return s
	}
...
// Apply the given function to each element of the series.
func (s Durations) MapNull(f aargh.MapFuncNull) Series {
	if len(s.Data_) == 0 {
		return s
	}
