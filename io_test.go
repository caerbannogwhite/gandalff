...
func Test_IOAll(t *testing.T) {
	...
	if series := df.Series("CHARVAR1"); !series.Eq(charvar1).(SeriesBool).All() {
		t.Errorf("expected %s, got %s", charvar1.DataAsString(), series.DataAsString())
	}

	if series := df.Series("CHARVAR2"); !series.Eq(charvar2).(SeriesBool).All() {
		t.Errorf("expected %s, got %s", charvar2.DataAsString(), series.DataAsString())
	}

	if series := df.Series("NUMVAR1"); !series.Eq(numvar1).(SeriesBool).All() {
		t.Errorf("expected %s, got %s", numvar1.DataAsString(), series.DataAsString())
	}

	if series := df.Series("FOO"); !series.Eq(numvar2).(SeriesBool).All() {
