...
func Benchmark_100000Rows_Filter(b *testing.B) {
	...
	for i := 0; i < b.N; i++ {
		df.Filter(
			df.C("Country").Map(func(v any) any { return v.(string) == "United States of America" }).(SeriesBool).
				And(
					df.C("Founded").Map(func(v any) any { return v.(int64) >= 2000 })).(SeriesBool).
				And(
					df.C("Number of employees").Map(func(v any) any { return v.(int64) < 1000 })).(SeriesBool),
		)
	}
	b.StopTimer()
}
...
func Test_BaseDataFrame_Sort(t *testing.T) {
	...
	if !checkEqSliceInt64(res.C("A").(SeriesInt64).Int64s(), []int64{1, 1, 1, 1, 1, 2, 2, 4, 5, 5}, nil, "") {
		t.Error("BaseDataFrame Sort, column A asc failed")
	}

	res = df.OrderBy(Desc("A"))
	if !checkEqSliceInt64(res.C("A").(SeriesInt64).Int64s(), []int64{5, 5, 4, 2, 2, 1, 1, 1, 1, 1}, nil, "") {
		t.Error("BaseDataFrame Sort, column A desc failed")
	}

	res = df.OrderBy(Asc("B"))
	if !checkEqSliceString(res.C("B").(SeriesString).Strings(), []string{"a", "a", "b", "b", "c", "c", "d", "e", "f", "g"}, nil, "") {
		t.Error("BaseDataFrame Sort, column B asc failed")
	}

	res = df.OrderBy(Desc("B"))
	if !checkEqSliceString(res.C("B").(SeriesString).Strings(), []string{"g", "f", "e", "d", "c", "c", "b", "b", "a", "a"}, nil, "") {
		t.Error("BaseDataFrame Sort, column B desc failed")
	}

	res = df.OrderBy(Asc("C"))
	if !checkEqSliceFloat64(res.C("C").(SeriesFloat64).Float64s(), []float64{1.2, 1.2, 2.3, 2.3, 3.4, 3.4, 4.5, 5.6, 7.8, 8.9}, nil, "") {
		t.Error("BaseDataFrame Sort, column C asc failed")
	}

	res = df.OrderBy(Desc("C"))
	if !checkEqSliceFloat64(res.C("C").(SeriesFloat64).Float64s(), []float64{8.9, 7.8, 5.6, 4.5, 3.4, 3.4, 2.3, 2.3, 1.2, 1.2}, nil, "") {
		t.Error("BaseDataFrame Sort, column C desc failed")
	}

	res = df.OrderBy(Asc("D"))
	if !checkEqSliceBool(res.C("D").(SeriesBool).Bools(), []bool{false, false, false, false, true, true, true, true, true, true}, nil, "") {
		t.Error("BaseDataFrame Sort, column D asc failed")
	}

	res = df.OrderBy(Desc("D"))
	if !checkEqSliceBool(res.C("D").(SeriesBool).Bools(), []bool{true, true, true, true, true, true, false, false, false, false}, nil, "") {
	...
	if !checkEqSliceString(res.C("B").(SeriesString).Strings(), []string{"a", "a", "c", "d", "f", "b", "c", "e", "b", "g"}, nil, "") {
	...
	if !checkEqSliceString(res.C("B").(SeriesString).Strings(), []string{"f", "d", "c", "a", "a", "c", "b", "e", "g", "b"}, nil, "") {
	...
	if !checkEqSliceString(res.C("B").(SeriesString).Strings(), []string{"b", "g", "e", "b", "c", "a", "a", "c", "d", "f"}, nil, "") {
	...
	if !checkEqSliceString(res.C("B").(SeriesString).Strings(), []string{"g", "b", "e", "c", "b", "f", "d", "c", "a", "a"}, nil, "") {
	...
	if !checkEqSliceFloat64(res.C("C").(SeriesFloat64).Float64s(), []float64{1.2, 1.2, 3.4, 4.5, 7.8, 2.3, 3.4, 5.6, 2.3, 8.9}, nil, "") {
	...
	if !checkEqSliceFloat64(res.C("C").(SeriesFloat64).Float64s(), []float64{7.8, 4.5, 3.4, 1.2, 1.2, 3.4, 2.3, 5.6, 8.9, 2.3}, nil, "") {
	...
	if !checkEqSliceFloat64(res.C("C").(SeriesFloat64).Float64s(), []float64{2.3, 8.9, 5.6, 2.3, 3.4, 1.2, 1.2, 3.4, 4.5, 7.8}, nil, "") {
	...
	if !checkEqSliceFloat64(res.C("C").(SeriesFloat64).Float64s(), []float64{8.9, 2.3, 5.6, 3.4, 2.3, 7.8, 4.5, 3.4, 1.2, 1.2}, nil, "") {
	...
	if !checkEqSliceBool(res.C("D").(SeriesBool).Bools(), []bool{false, true, false, true, true, true, true, false, false, true}, nil, "") {
	...
	if !checkEqSliceBool(res.C("D").(SeriesBool).Bools(), []bool{true, false, false, true, true, true, true, false, false, true}, nil, "") {
	...
	if !checkEqSliceBool(res.C("D").(SeriesBool).Bools(), []bool{true, true, false, false, true, true, true, false, true, false}, nil, "") {
	...
	if !checkEqSliceBool(res.C("D").(SeriesBool).Bools(), []bool{true, true, false, true, false, true, true, false, true, false}, nil, "") {
	...
	if !checkEqSliceInt64(res.C("A").(SeriesInt64).Int64s(), []int64{1, 2, 2, 1, 1, 5, 1, 5, 1, 4}, nil, "") {
		t.Error("BaseDataFrame Sort D desc, C asc, B desc: A failed")
	}
	if !checkEqSliceString(res.C("B").(SeriesString).Strings(), []string{"a", "b", "c", "d", "f", "g", "a", "b", "c", "e"}, nil, "") {
		t.Error("BaseDataFrame Sort D desc, C asc, B desc: B failed")
	}
	if !checkEqSliceFloat64(res.C("C").(SeriesFloat64).Float64s(), []float64{1.2, 2.3, 3.4, 4.5, 7.8, 8.9, 1.2, 2.3, 3.4, 5.6}, nil, "") {
...
func Test_BaseDataFrame_Sort_Nulls(t *testing.T) {
	...
	res = df.OrderBy(Asc("A"))
	if !checkEqSliceInt64(res.C("A").(SeriesInt64).Int64s(), []int64{1, 1, 1, 2, 4, 4, 4, 1, 2, 1}, nil, "") {
	...
	res = df.OrderBy(Desc("A"))
	if !checkEqSliceInt64(res.C("A").(SeriesInt64).Int64s(), []int64{1, 2, 1, 4, 4, 4, 2, 1, 1, 1}, nil, "") {
	...
	res = df.OrderBy(Asc("A"), Asc("B"))
	if !checkEqSliceInt64(res.C("A").(SeriesInt64).Int64s(), []int64{1, 1, 1, 2, 4, 4, 4, 2, 1, 1}, nil, "") {
		t.Error("BaseDataFrame Sort A asc, B asc: A failed")
	}
	if !checkEqSliceBool(res.C("A").GetNullMask(), []bool{false, false, false, false, false, false, false, true, true, true}, nil, "") {
		t.Error("BaseDataFrame Sort A asc: A nullmask failed")
	...
	if !checkEqSliceString(res.C("B").(SeriesString).Strings(), []string{"a", "d", NA_TEXT, "c", "e", "g", NA_TEXT, "b", "c", NA_TEXT}, nil, "") {
		t.Error("BaseDataFrame Sort A asc, B asc: B failed")
	}
	...
	res = df.OrderBy(Asc("A"), Desc("B"))
	if !checkEqSliceInt64(res.C("A").(SeriesInt64).Int64s(), []int64{1, 1, 1, 2, 4, 4, 4, 1, 1, 2}, nil, "") {
		t.Error("BaseDataFrame Sort A asc, B asc: A failed")
	}
	if !checkEqSliceBool(res.C("A").GetNullMask(), []bool{false, false, false, false, false, false, false, true, true, true}, nil, "") {
		t.Error("BaseDataFrame Sort A asc: A nullmask failed")
	...
	if !checkEqSliceString(res.C("B").(SeriesString).Strings(), []string{NA_TEXT, "d", "a", "c", NA_TEXT, "g", "e", NA_TEXT, "c", "b"}, nil, "") {
		t.Error("BaseDataFrame Sort A asc, B asc: B failed")
	}
